      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm text-slate-400">
            Short summary of the role, main responsibilities, and tech stack.
          </p>
        </div>

        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <AiOutputLanguageSelect
            id={`ai-lang-summary-${jobId}`}
            lang={lang}
            onChange={setLang}
            disabled={loading}
          />
          <button
            ...
            disabled={loading}
            className="shrink-0 rounded-2xl bg-white px-4 py-3 text-sm font-medium text-slate-950 transition hover:opacity-90 disabled:opacity-60"
          >
            {loading
              ? "Generating..."
              : result
